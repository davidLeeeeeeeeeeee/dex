...
func TestFlush1kLeaves(t *testing.T) {
	...
	go func() {
		...
		root.(*InternalNode).Flush(nil, flush)
		close(flushCh)
	}()
...
func TestDeleteHash(t *testing.T) {
	...
	tree.(*InternalNode).FlushAtDepth(0, nil, func(path []byte, vn VerkleNode) {})
	tree.Commit()
...
func TestDeleteResolve(t *testing.T) {
	...
	tree.(*InternalNode).FlushAtDepth(0, nil, saveNode)
	tree.Commit()
...
func TestGetResolveFromHash(t *testing.T) {
	...
	root.(*InternalNode).FlushAtDepth(0, nil, flush)
...
func TestInsertIntoHashedNode(t *testing.T) {
	...
	root.(*InternalNode).FlushAtDepth(0, nil, func(_ []byte, n VerkleNode) {})
	if err := root.Insert(fourtyKeyTest, zeroKeyTest, nil); err != nil {
		t.Fatalf("inserting into the original failed: %v", err)
	}
...
func TestInsertResolveSplitLeaf(t *testing.T) {
	...
	root.(*InternalNode).Flush(nil, func(_ []byte, node VerkleNode) {
		l, ok := node.(*LeafNode)
		if !ok {
			return
		}
