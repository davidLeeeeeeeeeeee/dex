...
func (n *InternalNode) InsertValuesAtStem(stem Stem, values [][]byte, resolver NodeResolverFn) error {
	...
	case HashedNode:
		...
		if child.commitment != nil {
			...
			resolveKey = stem[:n.depth+1]
		}
		serialized, err := resolver(resolveKey)
		if err != nil {
...
func (n *InternalNode) GetValuesAtStem(stem Stem, resolver NodeResolverFn) ([][]byte, error) {
	...
	case HashedNode:
		if resolver == nil {
			var hashStr string
			if child.commitment != nil {
				hashStr = fmt.Sprintf("%x", child.commitment.Bytes())
			} else {
				hashStr = "unknown"
			}
			return nil, fmt.Errorf("hashed node %s at depth %d along stem %x could not be resolved: %w", hashStr, n.depth, stem, errReadFromInvalid)
		}
		var resolveKey []byte
		if child.commitment != nil {
			comm := child.commitment.Bytes()
...
func (n *InternalNode) Delete(key []byte, resolver NodeResolverFn) (bool, error) {
	...
	case HashedNode:
		...
		if child.commitment != nil {
			...
			resolveKey = key[:n.depth+1]
		}
		payload, err := resolver(resolveKey)
		if err != nil {
			return false, err
		}
...
func (n *InternalNode) Flush(path []byte, flush NodeFlushFn) {
	n.Commit()
	for i, child := range n.children {
		...
		if c, ok := child.(*InternalNode); ok {
			c.Commit()
			c.Flush(childPath, flush)
			n.children[i] = HashedNode{commitment: c.Commitment()}
...
// is running scarce.
func (n *InternalNode) FlushAtDepth(depth uint8, path []byte, flush NodeFlushFn) {
	for i, child := range n.children {
		childPath := make([]byte, len(path)+1)
		copy(childPath, path)
		childPath[len(path)] = byte(i)
		...
		if n.depth < depth {
			c.FlushAtDepth(depth, childPath, flush)
			continue
		}

		child.Commit()
		c.Flush(childPath, flush)
		n.children[i] = HashedNode{commitment: child.Commitment()}
	}
}
	...
	for i, child := range n.children {
		...
		if child != nil {
			...
			if hn, ok := child.(HashedNode); ok {
				...
				var resolveKey []byte
				if hn.commitment != nil {
					comm := hn.commitment.Bytes()
					resolveKey = comm[:]
				} else {
					resolveKey = childpath
				}
				serialized, err := resolver(resolveKey)
				if err != nil {
