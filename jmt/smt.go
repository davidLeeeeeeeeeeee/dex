...
var errCorruptedTree = errors.New("corrupted tree: invalid node data")
...
// UpdateBatch updates multiple keys in the tree in a single operation.
func (smt *SparseMerkleTree) UpdateBatch(keys [][]byte, values [][]byte) ([]byte, error) {
	...
	for i := 0; i < len(keys); i++ {
		var err error
		currentRoot, err = smt.UpdateForRoot(keys[i], values[i], currentRoot)
		if err != nil {
			return nil, err
		}
	}
	smt.root = currentRoot
...
func (smt *SparseMerkleTree) sideNodesForRoot(path []byte, root []byte, getSiblingData bool) ([][]byte, [][]byte, []byte, []byte, error) {
	...
	for i := 0; i < smt.depth(); i++ {
		...
		if leftNode == nil || rightNode == nil {
			return nil, nil, nil, nil, errCorruptedTree
		}
...
func (smt *SparseMerkleTree) doProveForRoot(key []byte, root []byte, isUpdatable bool) (SparseMerkleProof, error) {
	...
	if !bytes.Equal(pathNodes[0], smt.th.placeholder()) {
		...
		if actualPath == nil {
			return SparseMerkleProof{}, errCorruptedTree
		}
...
func (smt *SparseMerkleTree) ProveCompactForRoot(key []byte, root []byte) (SparseCompactMerkleProof, error) {
	...
	compactedProof, err := CompactProof(proof, h)
