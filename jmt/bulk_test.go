...
func bulkCheckAll(t *testing.T, smt *SparseMerkleTree, kv *map[string]string) {
	for k, v := range *kv {
		...
		if !VerifyProof(proof, smt.Root(), []byte(k), []byte(v), h) {
			smt.th.hasherPool.Put(h)
		...
		h2 := smt.th.hasherPool.Get().(hash.Hash)
		if !VerifyCompactProof(compactProof, smt.Root(), []byte(k), []byte(v), h2) {
			smt.th.hasherPool.Put(h2)
			t.Error("Merkle proof failed to verify")
		}
		smt.th.hasherPool.Put(h2)
