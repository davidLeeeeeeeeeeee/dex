...
func TestProofsBasic(t *testing.T) {
	...
	h := smt.th.hasherPool.Get().(hash.Hash)
	defer smt.th.hasherPool.Put(h)

	// Generate and verify a proof on an empty key.
	proof, err = smt.Prove([]byte("testKey3"))
	checkCompactEquivalence(t, proof, h)
	if err != nil {
		t.Error("error returned when trying to prove inclusion on empty key")
	}
	result = VerifyProof(proof, bytes.Repeat([]byte{0}, smt.th.pathSize()), []byte("testKey3"), defaultValue, h)
	if !result {
		t.Error("valid proof on empty key failed to verify")
	}
	result = VerifyProof(proof, root, []byte("testKey3"), []byte("badValue"), h)
	if result {
	...
	result = VerifyProof(proof, root, []byte("testKey"), []byte("testValue"), h)
	if !result {
		t.Error("valid proof failed to verify")
	}
	result = VerifyProof(proof, root, []byte("testKey"), []byte("badValue"), h)
	if result {
		t.Error("invalid proof verification returned true")
	}
	...
	result = VerifyProof(randomiseProof(proof), root, []byte("testKey"), []byte("testValue"), h)
	if result {
		t.Error("invalid proof verification returned true")
	}
	...
	result = VerifyProof(proof, root, []byte("testKey2"), []byte("testValue"), h)
	if !result {
		t.Error("valid proof failed to verify")
	}
	result = VerifyProof(proof, root, []byte("testKey2"), []byte("badValue"), h)
	if result {
		t.Error("invalid proof verification returned true")
	}
	result = VerifyProof(randomiseProof(proof), root, []byte("testKey2"), []byte("testValue"), h)
	if result {
		t.Error("invalid proof verification returned true")
	}
	...
	result = VerifyProof(proof, root, []byte("testKey2"), defaultValue, h)
	if result {
		t.Error("invalid proof verification returned true")
	}
	...
	result = VerifyProof(proof, root, []byte("testKey3"), defaultValue, h)
	if !result {
		t.Error("valid proof on empty key failed to verify")
	}
	result = VerifyProof(proof, root, []byte("testKey3"), []byte("badValue"), h)
	...
	result = VerifyProof(randomiseProof(proof), root, []byte("testKey3"), defaultValue, h)
	if result {
		t.Error("invalid proof verification returned true")
	}
}
...
func TestProofsSanityCheck(t *testing.T) {
	...
	result := VerifyProof(proof, root, []byte("testKey1"), []byte("testValue1"), h)
	if result {
		t.Error("invalid proof verification returned true")
	}
	_, err := CompactProof(proof, h)
	if err == nil {
...
func TestCompactProofsSanityCheck(t *testing.T) {
	...
	result := VerifyCompactProof(proof, root, []byte("testKey1"), []byte("testValue1"), h)
	if result {
		t.Error("invalid proof verification returned true")
	}
