...
// planAccountChainJobWindow 账户链/合约链规划（支持批量）
...
func (p *JobWindowPlanner) planBTCJob(chain, asset string, vaultID uint32, keyEpoch uint64, withdraws []*ScanResult, totalAmount uint64) (*Job, error) {
	// 1. 读取所有 withdraw 详情
	outputs := make([]chain.WithdrawOutput, 0, len(withdraws))
	...
	for i, wd := range withdraws {
		...
		outputs = append(outputs, chain.WithdrawOutput{
...
func (p *JobWindowPlanner) selectBTCUTXOs(chain string, vaultID uint32, needAmount uint64) ([]chain.UTXO, error) {
	// 扫描该 Vault 的所有 UTXO
	utxoPrefix := fmt.Sprintf("v1_frost_btc_utxo_%d_", vaultID)
	utxos := make([]chain.UTXO, 0)
