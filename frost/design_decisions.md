...
### 1. Threshold 门限公式
- **位置**：0.术语 L26
- **原文**：`建议默认 t = ceil(N * 0.8)，可配置`
- **需决策**：确定 threshold 公式，是否允许运行时配置？
- [ ] **决策**：_______________
- **备注**：
...
- **位置**：3.目录 L116
- **原文**：`建议逐步迁移到 core 子目录`
- **需决策**：现有 DKG 代码迁移到 `frost/core/*` 还是保持现状？
- [ ] **决策**：_______________
- **备注**：

### 3. Key 前缀命名规范
- **位置**：4.2 L195-197
- **原文**：`Key 前缀（建议）`、`用 v1_frost_ 举例`
- **需决策**：确定最终 Key 前缀（如 `v1_frost_` 或其他）
- [ ] **决策**：_______________
- **备注**：

### 4. 数据结构定义方式
- **位置**：4.3 L209
- **原文**：`建议 Proto / Go struct`
- **需决策**：用 Protobuf 还是纯 Go struct？
- [ ] **决策**：[ ] Proto  [ ] Go struct  [ ] 两者都用
- **备注**：

### 5. withdraw_id 生成方式
- **位置**：4.3.1 L213
- **原文**：`建议：hash(txid || output_index) 或 VM 自增序列`
- **需决策**：二选一
- [ ] **决策**：[ ] hash(txid || output_index)  [ ] VM 自增序列
- **备注**：
...
### 7. migration_sig_count 字段
- **位置**：4.3.3 L248
- **原文**：`migration_sig_count：（可选）`
- **需决策**：TransitionState 是否包含此字段？
- [ ] **决策**：[ ] 包含  [ ] 不包含
- **备注**：
...
### 9. 权力交接触发阈值
- **位置**：6.1 L335
- **原文**：`v1 推荐：changed_count >= 2000（即 20% 变更）`
- **需决策**：触发阈值是 20%（2000人）还是 80%（8000人）？
- [ ] **决策**：[ ] 20% (2000)  [ ] 80% (8000)  [ ] 其他：___
- **备注**：requirements 中表述不一致，需明确

### 10. 合约链托管策略
- **位置**：6.3 L358
- **原文**：`建议统一用"托管合约/程序"管理资产与 signer pubkey`
- **需决策**：确认采纳此策略？
- [ ] **决策**：[ ] 采纳  [ ] 不采纳，改用：_______________
- **备注**：

### 11. 提现与交接并行策略
- **位置**：6.4 L383
- **原文**：`v1 推荐策略 A：短暂停顿（秒~分钟级）即可`
- **需决策**：策略 A（切换前暂停新签名）还是策略 B（双 key 过渡期）？
- [ ] **决策**：[ ] 策略 A  [ ] 策略 B
- **备注**：

### 12. 合约层去重
- **位置**：7.2 L399
- **原文**：`强烈建议 v1 用"合约/程序层去重"`
- **需决策**：确认采纳？
- [ ] **决策**：[ ] 采纳  [ ] 不采纳
- **备注**：
