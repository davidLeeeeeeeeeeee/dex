...
//      若仲裁在规定时间内（如 24h）未达成共识，将触发“扩大范围”机制，引入更多节点参与仲裁。
//      随着范围扩大，仲裁通过的阈值将从 80% 逐步降低，直到全体见证者参与时的 50%。
//    - 惩罚执行：
...
// 		如果通过、未通过都没有一方超过80%（到截止时间，或者全部人都已经投票，或者已经提前计算出无法达成共识,即弃票超过20%）->扩大采样范围。
//      一旦到截止期出现了矛盾结果（既有通过，也有未通过投票），不管是否有超过80%情况，自动进入挑战阶段。
...
// 问题1、pending的资产该作何处理？
// 答:签名状态有,通过、未通过（资金不符合，hash不匹配）、弃票（无法判断当前状态，不会被追责）。矿工可以选择弃票签名，若签名了后期恶意的tx,可能被发起挑战而损失矿工自己资金。
// 问题2、如何解决冒领的问题？
// 转账发起方填写memo 指定收款地址。
